...
func (s *Service) Create(id, name, description string) (models.Benchmark, error) {
	start := time.Now()
	if id == "" || name == "" {
		s.observe("create", start, "error")
		return models.Benchmark{}, errors.New("missing fields")
	}
	benchmark := models.Benchmark{ID: id, Name: name, Description: description, CreatedAt: time.Now()}
	s.repo.Save(benchmark)
	if s.log != nil {
		s.log.Printf("benchmark: created benchmark %s", id)
	}
